...
#include </opt/strands/strands_catkin_ws/devel/include/scitos_msgs/ChargerStatus.h>
...
/*   if (state == STATE_IDLE){
   ...
   if (state == STATE_APPROACH || state == STATE_MEASURE || state == STATE_DOCK || state == STATE_ADJUST || state == STATE_WAIT || state == STATE_CALIBRATE){
